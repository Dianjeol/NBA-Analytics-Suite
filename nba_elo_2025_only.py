#!/usr/bin/env python3
"""
NBA Elo Rating Calculator for 2025 Games Only
...
class EloCalculator2025:
    ...
    def calculate_k_factor(self, game_number, total_games):
        ...
        progress = game_number / total_games if total_games > 0 else 0
...
def format_elo_display_2025(team_elos, title):
...
def create_conference_divisions_2025():
...
def main():
    print("🏀 NBA Elo Calculator - 2025 Games Only")
    print("="*70)
    ...
    # Process all 2025 games
    print(f"\nProcessing {total_games} games from 2025...")
    for i, game in enumerate(games_2025):
        k_factor = elo_calc.calculate_k_factor(i, total_games)
    ...
    format_elo_display_2025(elo_calc.team_elos, "FINAL ELO RATINGS - 2025 GAMES ONLY")
    
    # Show statistics
    print(f"\n{'='*70}")
    print("2025 SEASON STATISTICS")
    print(f"{'='*70}")
    ...
    if teams_that_played:
        ...
        team_conferences = create_conference_divisions_2025()
    ...
    for i, game in enumerate(games_2025[:5]):
        stage_name = "Preseason" if game['stage'] == 1 else "Regular Season" if game['stage'] == 2 else "Playoffs"
        print(f"{game['date'].strftime('%Y-%m-%d')}: {game['visitor_team']} @ {game['home_team']} "
              f"({game['visitor_score']}-{game['home_score']}) - {stage_name}")
    
    if len(games_2025) > 5:
        print("...")
        last_game = games_2025[-1]
        stage_name = "Preseason" if last_game['stage'] == 1 else "Regular Season" if last_game['stage'] == 2 else "Playoffs"
        print(f"{last_game['date'].strftime('%Y-%m-%d')}: {last_game['visitor_team']} @ {last_game['home_team']} "
              f"({last_game['visitor_score']}-{last_game['home_score']}) - {stage_name}")
